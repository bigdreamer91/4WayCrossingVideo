//Geethanjali Jeevanatham, Anupa Prudhivi, Sindhusha Dhulipala, Arko Basureated by Rafael Radkowski on 9/26/15.
//
//
...
GLPlane3D::GLPlane3D(float center_x, float center_y, float center_z, float width, float height, int val, int rows, int cols  ):
    _center_x(center_x), _center_y(center_y), _center_z(center_z), _width(width), _height(height),val(val), _rows(rows), _cols(cols)
{
...
void GLPlane3D::make_box(float center_x, float center_y, float center_z, float width,
                        float height, int val, int rows, int cols,
                        vector<Vertex>& vp, vector<Vertex>& nv )
{
    ...
    nv = vector<Vertex>(4);
        ...
        vp[0].x() = center_x;
        vp[0].y() = -_height/2.0 + center_y;
        vp[0].z() = -_width/2.0 + center_z;
        vp[0].u() = 0.0;
        vp[0].v() = 0.0;
        
        vp[1].x() = center_x;
        vp[1].y() = _height/2.0 + center_y;
        vp[1].z() = -_width/2.0 + center_z;
        vp[1].u() = 0.0;
        vp[1].v() = 1.0;
        
        vp[2].x() = center_x;
        vp[2].y() = -_height/2.0 + center_y;
        vp[2].z() = _width/2.0 + center_z;
        vp[2].u() = 1.0;
        vp[2].v() = 0.0;
        
        vp[3].x() = center_x;
        vp[3].y() = _height/2.0 + center_y;
        vp[3].z() = _width/2.0 + center_z;
        vp[3].u() = 1.0;
        vp[3].v() = 1.0;
    }
    else if(val==3){
        vp[0].x() = -_width/2.0 + center_x;
        vp[0].y() = center_y;
        vp[0].z() = -_height/2.0 + center_z;
        vp[0].u() = 0.0;
        vp[0].v() = 0.0;
        
        vp[1].x() = -_width/2.0 + center_x;
        vp[1].y() =  center_y;
        vp[1].z() = _height/2.0 + center_z;
        vp[1].u() = 0.0;
        vp[1].v() = 1.0;
        
        vp[2].x() = _width/2.0 + center_x;
        vp[2].y() = center_y;
        vp[2].z() = -_height/2.0 + center_z;
        vp[2].u() = 1.0;
        vp[2].v() = 0.0;
        
        vp[3].x() = _width/2.0 + center_x;
        vp[3].y() =  center_y;
        vp[3].z() = _height/2.0 + center_z;
        vp[3].u() = 1.0;
        vp[3].v() = 1.0;
    }
    else{
