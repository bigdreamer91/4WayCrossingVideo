//Anupa Prudhivi, Geethanjali Jeevanatham, Arko Basu, Sindhusha Dhulipala
...
GLBox3D::GLBox3D(float center_x, float center_y, float center_z, float width, float height, float length, int rows, int cols) :
_center_x(center_x), _center_y(center_y), _center_z(center_z), _width(width), _height(height), _length(length), _rows(rows), _cols(cols)
{
      _program = 0;
