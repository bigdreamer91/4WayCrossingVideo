...
bool GLObjectObj::load_obj(const char* filename, vector<glm::vec3> &vertices, vector<glm::vec3> &normals, vector<glm::vec2> &textures, vector<GLuint> &elements)
{
    ...
    while (getline(in, line))
    {
        if (line.substr(0,2) == "v ")
        {
            ...
            glm::vec2 v; s >> v.x; s >> v.y;// v.w = 1.0f;
    ...
    for(int i=0; i<temp_index_triangle.size(); i++)
    {
        ...
        glm::vec3 tempIdx = temp_index_textures[i];
        
       // cout << tempIdx.x << "  " << tempIdx.y << "  " << tempIdx.z <<  "  " << temp_textures.size() <<endl;
        ...
        textures.push_back(temp_textures[tempIdx.x-1]);
        textures.push_back(temp_textures[tempIdx.y-1]);
        textures.push_back(temp_textures[tempIdx.z-1]);
...
void GLObjectObj::initVBO(void)
{
    ...
    float* textures = new float[_textures.size() * 2];
   ...
   // glBufferData(GL_ELEMENT_ARRAY_BUFFER, _elements.size() * sizeof(unsigned int), &_elements[0], GL_STATIC_DRAW);
    ...
    int locText = glGetAttribLocation(_program, "in_TexCoord");
    glBindBuffer(GL_ARRAY_BUFFER, _vboID[2]); // Bind our Vertex Buffer Object
    glBufferData(GL_ARRAY_BUFFER, _textures.size() * 2 * sizeof(GLfloat), &textures[0], GL_STATIC_DRAW); // Set the size and data of our VBO and set it to STATIC_DRAW
    
    glVertexAttribPointer((GLuint)locText, 2, GL_FLOAT, GL_FALSE, 0, 0); // Set up our vertex attributes pointer
    glEnableVertexAttribArray(locText);
